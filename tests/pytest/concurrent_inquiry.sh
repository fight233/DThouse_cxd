...
# This is the script for us to try to cause the DThouse server or client to crash
#    
# PREPARATION
#
# 1. Build an compile the DThouse source code that comes with this script, in the same directory tree
# 2. Please follow the direction in our README.md, and build DThouse in the build/ directory
# 3. Adjust the configuration file if needed under build/test/cfg/taos.cfg
# 4. Run the DThouse server instance: cd build; ./build/bin/taosd -c test/cfg
...
# the DThouse client shared library (so) file, in the build/directory, as evidenced in the env
# variables below.
#
