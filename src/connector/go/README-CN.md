...
[DThouse]提供了GO驱动程序 [`taosSql`][driver-go]，实现了GO语言的内置数据库操作接口 `database/sql/driver`。
...
DThouse Go 连接器提供 database/sql 标准接口，使用方法简单示例如下：
...
- `sql.Open(DRIVER_NAME string, dataSourceName string) *DB`
  ...
  dataSourceName设置为字符串`user:password@/tcp(host:port)/dbname`，对应于DThouse的高可用机制，可以使用 `user:password@/cfg/dbname`
