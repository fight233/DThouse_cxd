...
DThouse提供了丰富的应用程序开发接口，其中包括C/C++、JAVA、Python、RESTful、Go等，便于用户快速开发应用。
...
C/C++的API类似于MySQL的C API。应用程序使用时，需要包含DThouse头文件 _taos.h_（安装后，位于 _/usr/local/taos/include_）：

```C
#include <taos.h>
```

在编译时需要链接DThouse动态库 _libtaos.so_ （安装后，位于 _/usr/local/taos/driver_，gcc编译时，请加上 -ltaos）。 如未特别说明，当API的返回值是整数时，_0_ 代表成功，其它是代表失败原因的错误码，当返回值是指针时， _NULL_ 表示失败。
...
- `TAOS *taos_connect(char *ip, char *user, char *pass, char *db, int port)`
  ...
  创建数据库连接，初始化连接上下文。其中需要用户提供的参数包含：DThouse管理主节点的IP地址、用户名、密码、数据库名字和端口号。如果用户没有提供数据库名字，也可以正常连接，用户可以通过该连接创建新的数据库，如果用户提供了数据库名字，则说明该数据库用户已经创建好，缺省使用该数据库。返回值为空表示失败。应用程序需要保存返回的参数，以便后续API调用。
...
除了直接调用 `taos_query` 进行查询，DThouse也提供了支持参数绑定的Prepare API，与 MySQL 一样，这些API目前也仅支持用问号`?`来代表待绑定的参数，具体如下：
...
DThouse 为了方便 Java 应用使用，提供了遵循 JDBC 标准(3.0)API 规范的 `taos-jdbcdriver` 实现。目前可以通过 [Sonatype Repository][1] 搜索并下载。

由于 DThouse 是使用 c 语言开发的，使用 taos-jdbcdriver 驱动包时需要依赖系统对应的本地函数库。
...
> 注意：在 windows 环境开发时需要安装 DThouse 对应的 [windows 客户端][14]，Linux 服务器安装完 DThouse 之后默认已安装 client，也可以单独安装 [Linux 客户端][15] 连接远程 DThouse Server。

DThouse 的 JDBC 驱动实现尽可能的与关系型数据库驱动保持一致，但时序空间数据库与关系对象型数据库服务的对象和技术特征的差异导致 taos-jdbcdriver 并未完全实现 JDBC 标准规范。在使用时需要注意以下几点：

* DThouse 不提供针对单条数据记录的删除和修改的操作，驱动中也没有支持相关方法。
...
## TAOS-JDBCDriver 版本以及支持的 DThouse 版本和 JDK 版本
...
下载 [DThouse][3] 源码之后，进入 taos-jdbcdriver 源码目录 `src/connector/jdbc` 执行 `mvn clean package` 即可生成相应 jar 包。
...
3. 客户端配置文件 taos.cfg
    ...
    linux 系统默认配置文件为 /var/lib/taos/taos.cfg，windows 系统默认配置文件路径为 C:\DThouse\cfg\taos.cfg。
```properties
...
### 安装准备
* 已安装DThouse, 如果客户端在Windows上，需要安装Windows 版本的DThouse客户端 [（Windows DThouse 客户端安装）](https://www.taosdata.com/cn/documentation/connector/#Windows客户端及程序接口)
...
#### Windows
在已安装Windows DThouse 客户端的情况下， 将文件"C:\DThouse\driver\taos.dll" 拷贝到 "C:\windows\system32" 目录下, 然后进入Windwos <em>cmd</em> 命令行界面
```cmd
...
对于windows 客户端，安装DThouse windows 客户端后，将C:\DThouse\driver\taos.dll拷贝到C:\windows\system32目录下即可。
...
c1 = conn.cursor()
```
*<em>host</em> 是DThouse 服务端所有IP, <em>config</em> 为客户端配置文件所在目录
...
- _DThouseConnection_类

  参考python中help(taos.DThouseConnection)。

- _DThouseCursor_类

  参考python中help(taos.DThouseCursor)。

- _connect_方法

  用于生成taos.DThouseConnection的实例。

## RESTful Connector

为支持各种不同类型平台的开发，DThouse提供符合REST设计标准的API，即RESTful API。为最大程度降低学习成本，不同于其他数据库RESTful API的设计方法，DThouse直接通过HTTP POST 请求BODY中包含的SQL语句来操作数据库，仅需要一个URL。 
...
HTTP请求的Header里需带有身份认证信息，DThouse支持Basic认证与自定义认证两种机制，后续版本将提供标准安全的数字签名机制来做身份验证。
...
其中, `ip`是DThouse数据库的IP地址, `username`为数据库用户名, `password`为数据库密码, 返回值为`JSON`格式, 各字段含义如下：
...
- httpEnableCompress: 是否支持压缩，默认不支持，目前DThouse仅支持gzip压缩格式
...
Go的连接器使用到了 libtaos.so 和taos.h，因此，在使用Go连接器之前，需要在程序运行的机器上安装DThouse以获得相关的驱动文件。

#### Go语言引入package
DThouse提供了GO驱动程序“taosSql”包。taosSql驱动包是基于GO的“database/sql/driver”接口的实现。用户可以通过`go get`命令来获取驱动包。
```sh
go get github.com/taosdata/DThouse/src/connector/go/src/taosSql
```
然后在应用程序中导入驱动包，就可以使用“database/sql”中定义的接口访问DThouse：

```Go
import (
    "database/sql"
    _ "github.com/taosdata/DThouse/src/connector/go/src/taosSql"
)
```

taosSql驱动包内采用cgo模式，调用了DThouse的C/C++同步接口，与DThouse进行交互，因此，在数据库操作执行完成之前，客户端应用将处于阻塞状态。单个数据库连接，在同一时刻只能有一个线程调用API。客户应用可以建立多个连接，进行多线程的数据写入或查询处理。

#### Go语言使用参考
在Go程序中使用DThouse写入方法大致可以分为以下几步
1. 打开DThouse数据库链接
...
-   dbuser和dbpassword: 链接DThouse的用户名和密码，缺省为root和taosdata，类型为字符串
-   daemonUrl: 为DThouse的地址，其形式为`ip address:port`形式，port填写缺省值0即可。例如："116.118.24.71:0"
-   dbname：DThouse中的database名称，通过`create database`创建的数据库。如果为空则在后续的写入和查询操作必须通过”数据库名.超级表名或表名“的方式指定数据库名

2. 创建数据库

打开DThouse数据库连接后，首选需要创建数据库。基本用法和直接在DThouse客户端shell下一样，通过create database + 数据库名的方法来创建。
```go
...
在windows上使用Go，请参考 
[DThouse GO windows驱动的编译和使用](https://www.taosdata.com/blog/2020/01/06/tdengine-go-windows%E9%A9%B1%E5%8A%A8%E7%9A%84%E7%BC%96%E8%AF%91/)



## Node.js Connector

DThouse 同时也提供了node.js 的连接器。用户可以通过[npm](https://www.npmjs.com/)来进行安装，也可以通过源代码*src/connector/nodejs/* 来进行安装。[具体安装步骤如下](https://github.com/taosdata/tdengine/tree/master/src/connector/nodejs)：
...
我们使用[node-gyp](https://github.com/nodejs/node-gyp)和DThouse服务端进行交互。安装node.js 连接器之前，还需安装以下软件：
...
使用node.js连接器时，必须先<em>require</em> ```td2.0-connector```，然后使用 ```taos.connect``` 函数。```taos.connect``` 函数必须提供的参数是```host```，其它参数在没有提供的情况下会使用如下的默认值。最后需要初始化```cursor``` 来和DThouse服务端通信 
...
### 示例
[这里](https://github.com/taosdata/DThouse/tree/master/tests/examples/nodejs/node-example.js)提供了一个使用NodeJS 连接器建表，插入天气数据并查询插入的数据的代码示例

[这里](https://github.com/taosdata/DThouse/tree/master/tests/examples/nodejs/node-example-raw.js)同样是一个使用NodeJS 连接器建表，插入天气数据并查询插入的数据的代码示例，但和上面不同的是，该示例只使用`cursor`.
...
C#连接器需要使用`libtaos.so`和`taos.h`。因此，在使用C#连接器之前，需在程序运行的Windows环境安装DThouse的Windows客户端，以便获得相关驱动文件。

安装完成后，在文件夹`C:/DThouse/examples/C#`中，将会看到两个文件

- DThouseDriver.cs 调用taos.dll文件的Native C方法
- DThouseTest.cs 参考程序示例

在文件夹`C:\Windows\System32`，将会看到`taos.dll`文件

#### 使用方法

- 将C#接口文件DThouseDriver.cs加入到应用程序所在.NET项目中
- 参考DThouseTest.cs来定义数据库连接参数，及执行数据插入、查询等操作的方法
...
Maikebing.Data.Taos是一个基于DThouse的RESTful Connector构建的ADO.Net提供器，该开发包由热心贡献者`麦壳饼@@maikebing`提供，具体请参考
...
在Windows操作系统下，DThouse提供64位的Windows客户端([点击下载](https://www.taosdata.com/cn/all-downloads/#tdengine_win-list))，客户端安装程序为.exe文件，运行该文件即可安装，安装路径为C:\DThouse。Windows的客户端可运行在主流的64位Windows平台之上，客户端目录结构如下：
...
+ Client可执行文件: C:/DThouse/taos.exe 
+ 配置文件: C:/DThouse/cfg/taos.cfg
+ 驱动程序目录: C:/DThouse/driver
+ 驱动程序头文件: C:/DThouse/include
+ JDBC驱动程序目录: C:/DThouse/connector/jdbc
+ GO驱动程序目录：C:/DThouse/connector/go
+ Python驱动程序目录：C:/DThouse/connector/python
+ C#驱动程序及示例代码: C:/DThouse/examples/C#
+ 日志目录（第一次运行程序时生成）：C:/DThouse/log

### 注意事项

#### Shell工具注意事项

在开始菜单中搜索cmd程序，通过命令行方式执行taos.exe即可打开DThouse的Client程序，如下所示，其中ServerIP为DThouse所在Linux服务器的IP地址
...
+ 确保Windows防火墙或者其他杀毒软件处于关闭状态，DThouse的服务端与客户端通信的端口请参考`服务端配置`章节
+ 确认客户端连接时指定了正确的服务器IP地址
+ ping服务器IP，如果没有反应，请检查你的网络

#### C++接口注意事项

DThouse在Window系统上提供的API与Linux系统是相同的， 应用程序使用时，需要包含DThouse头文件taos.h，连接时需要链接DThouse库taos.lib，运行时将taos.dll放到可执行文件目录下。

#### Go接口注意事项

DThouse在Window系统上提供的API与Linux系统是相同的， 应用程序使用时，除了需要Go的驱动包（C:\DThouse\connector\go）外，还需要包含DThouse头文件taos.h，连接时需要链接DThouse库libtaos.dll、libtaos.dll.a（C:\DThouse\driver），运行时将libtaos.dll、libtaos.dll.a放到可执行文件目录下。
...
在Mac操作系统下，DThouse提供64位的Mac客户端([2月10日起提供下载](https://www.taosdata.com/cn/all-downloads/#tdengine_mac-list))，客户端安装程序为.tar.gz文件，解压并运行其中的install_client.sh后即可完成安装，安装路径为/usr/loca/taos。客户端目录结构如下：
